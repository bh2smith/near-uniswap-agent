...
export async function orderRequestFlow({ chainId, quoteRequest, }) {
    const [sellToken, buyToken] = await Promise.all([
        getToken(chainId, quoteRequest.sellToken),
        getToken(chainId, quoteRequest.buyToken),
    ]);
    const route = await getRoute(chainId, quoteRequest.amount, sellToken, buyToken, quoteRequest.walletAddress);
    if (!route || !route.methodParameters) {
        // Handle failed request
        throw new Error(`Failed to get route on ${chainId} for ${JSON.stringify(quoteRequest)}`);
    }
...
export async function getToken(chainId, address) {
